...
const ELEMENT_TAGS = {
  ...
  TR: (el) => ({ type: 'tr', attrs: {} }),
  TD: (el) => ({ type: 'td', attrs: { ...spanningAttrs(el) } }),
  TH: (el) => ({ type: 'th', attrs: { ...spanningAttrs(el) } }),
...
const fromRedactor = (el, options) => {
  ...
  let children = flatten(Array.from(parent.childNodes).map((child) => fromRedactor(child, options)))
  ...
  if (ELEMENT_TAGS[nodeName]) {
    ...
    if (nodeName === 'FIGURE') {
      ...
      if (captionElements?.[0]) {
        ...
        if (captionElementsAttrs) {
          Array.from(captionElementsAttrs).forEach((child) => {
    ...
    if (nodeName === 'SPAN') {
      if (elementAttrs?.attrs?.["redactor-attributes"]?.['data-type'] === 'inlineCode') {
        ...
        return children.map((child) => jsx('text', attrsStyle, child))
      }
...
const getReferenceAttributes = ({ elementAttrs, newChildren, extraAttrs, sizeAttrs }) => {
...
const getFinalImageAttributes = ({ elementAttrs, newChildren, extraAttrs, sizeAttrs }) => {
...
const getNestedValueIfAvailable = (value) => {
  try {
...
function getTbodyChildren(rows) {
...
module.exports = traverseChildAndWarpChild;